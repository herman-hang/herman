...
func GenerateToken(claims *Claims) string {
	// token有效时间（纳秒）
	var effectTime = app.Config.Jwt.EffectTime * time.Hour
	//设置token有效期
	claims.ExpiresAt = time.Now().Add(effectTime).Unix()
	//生成token
	sign, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).SignedString([]byte(app.Config.Jwt.JwtSecret))
	if err != nil {
...
func ParseToken(tokenString string, ctx *gin.Context, guard string) *Claims {
	...
	token, err := jwt.ParseWithClaims(tokenString, &Claims{}, func(token *jwt.Token) (interface{}, error) {
		return []byte(app.Config.Jwt.JwtSecret), nil
	})
