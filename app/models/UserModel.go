...
// Users 用户表结构体
type Users struct {
	Id           uint           `json:"id" gorm:"column:id;type:uint(11);primary_key;comment:用户ID"`
	User         string         `json:"user" gorm:"column:user;type:varchar(20);unique;not null;comment:用户名"`
	Password     string         `json:"password" gorm:"column:password;type:char(60);not null;comment:用户密码"`
